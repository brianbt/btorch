...
def _make_radial_window(width, height, cx, cy, fn, window_width=10.0, device='cuda'):
    ...
    xs = torch.arange(width, dtype=torch.float32, device=device).view((1, width)).repeat(channels, height, 1)
    ys = torch.arange(height, dtype=torch.float32, device=device).view((height, 1)).repeat(channels, 1, width)
    delta_xs = xs - cx
    delta_ys = ys - cy
    dists = torch.sqrt((delta_ys ** 2) + (delta_xs ** 2))

    # apply the function to the grid and return it
    return fn(dists, window_width).to(device)
...
class SoftArgmax2D(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x_indices = torch.arange(start=self.base_index, end=x_end_index, step=self.step_size, device=x.device)
        ...
        y_indices = torch.arange(start=self.base_index, end=y_end_index, step=self.step_size, device=x.device)
